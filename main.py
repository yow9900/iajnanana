...
PREPEND_SILENCE_SEC=int(env.get("PREPEND_SILENCE_SEC","10"))
...
# === In-memory data structures replacing database ===
user_transcriptions={}
in_memory_data={
    "pending_media":{},
    "user_data":{} # Key: user_id, Value: {"stt_language": str, "stt_send_mode": str, "last_active": datetime}
}
action_usage={}
...
# === Database Replacement Functions ===
def get_user_data(user_id):
    uid=norm_user_id(user_id)
    with memory_lock: return in_memory_data["user_data"].get(uid,{})

def update_user_activity(user_id):
    uid=norm_user_id(user_id); now=datetime.now()
    with memory_lock:
        ud=in_memory_data["user_data"].setdefault(uid,{"stt_language":"en","stt_send_mode":"file","stt_conversion_count":0})
        ud["last_active"]=now
...
def increment_processing_count(user_id,service_type):
    ...
    with memory_lock:
        ud=in_memory_data["user_data"].setdefault(uid,{"stt_language":"en","stt_send_mode":"file","stt_conversion_count":0})
        key=f"{service_type}_conversion_count"
        ud[key]=ud.get(key,0)+1

def get_stt_user_lang(user_id):
    uid=norm_user_id(user_id)
    with memory_lock:
        return in_memory_data["user_data"].get(uid,{}).get("stt_language")

def set_stt_user_lang(user_id,lang_code):
    uid=norm_user_id(user_id)
    with memory_lock:
        ud=in_memory_data["user_data"].setdefault(uid,{"stt_language":"en","stt_send_mode":"file","stt_conversion_count":0})
        ud["stt_language"]=lang_code

def get_user_send_mode(user_id):
    uid=norm_user_id(user_id)
    with memory_lock:
        return in_memory_data["user_data"].get(uid,{}).get("stt_send_mode","file")
...
def set_user_send_mode(user_id,mode):
    ...
    with memory_lock:
        ud=in_memory_data["user_data"].setdefault(uid,{"stt_language":"en","stt_send_mode":"file","stt_conversion_count":0})
        ud["stt_send_mode"]=mode
# === End of Database Replacement Functions ===
...
def process_media_file(message,bot_obj,bot_token,bot_index,file_id,file_size,filename):
    ...
    # Check if lang is set (means user is not new or has selected language)
    if not lang:
        # If no language is set, prompt user to select one before processing
        save_pending_media(uid, "media", {"file_id": file_id, "file_size": file_size, "filename": filename})
        bot_obj.send_message(message.chat.id,"Choose your file language for transcription using the below buttons:",reply_markup=build_lang_keyboard("media_select_lang"))
        return

    # === FARIINTA XAYAYSIISKA EE CUSUB HADDAY TAHAY HALKAN AYAY KA BILAAJMaysaa ===
    migration_message = "🚀 Switch to New Bot Now: https://t.me/MediaToTextBot\n\n✨ Why should I use the new bot, and what makes it better than this?\n • ⚡ Lightning-fast transcription\n • 🎯 High accuracy\n • 📁 Upload files up to 2GB \n • More features like Cleaning up the text "
...
def register_handlers(bot_obj,bot_token,bot_index):
    ...
    @bot_obj.callback_query_handler(func=lambda c:c.data and c.data.startswith("media_select_lang|"))
    def media_select_lang_callback(call):
        try:
            uid=str(call.from_user.id); _,lang_code=call.data.split("|",1)
            lang_label=CODE_TO_LABEL.get(lang_code,lang_code)
            set_stt_user_lang(uid,lang_code)
            
            pending_media_data=pop_pending_media(uid)
            
            try: bot_obj.delete_message(call.message.chat.id,call.message.message_id)
            except: pass
            bot_obj.answer_callback_query(call.id,f"✅ Language set to {lang_label}. Processing file...")

            if pending_media_data:
                # Re-queue the media file for processing now that language is set
                message_template = call.message.reply_to_message
                file_id = pending_media_data["data"]["file_id"]
                file_size = pending_media_data["data"]["file_size"]
                filename = pending_media_data["data"]["filename"]
                
                # Reconstruct a simplified message object for queueing (needs improvement for robustness)
                # For simplicity, we use the callback message as a base, which works for private chats.
                message_to_queue = type('M', (object,), {
                    'from_user': call.from_user,
                    'chat': call.message.chat,
                    'message_id': message_template.message_id,
                    'voice': None, 'audio': None, 'video': None, 'document': None # Placeholder fields
                })
                
                # Attach the correct file object to the message_to_queue object
                if "voice" in filename:
                    message_to_queue.voice = type('V', (object,), {'file_id': file_id, 'file_size': file_size})
                elif "audio" in filename or "video" in filename: # Simplified check
                    message_to_queue.audio = type('A', (object,), {'file_id': file_id, 'file_size': file_size, 'file_name': filename})
                else:
                    message_to_queue.document = type('D', (object,), {'file_id': file_id, 'file_size': file_size, 'file_name': filename})

                with memory_lock:
                    PENDING_QUEUE.append((message_to_queue, bot_obj, bot_token, bot_index, file_id, file_size, filename))
            
        except:
            logging.exception("Error in media_select_lang_callback")
            try: bot_obj.answer_callback_query(call.id,"❌ Error setting language or processing file, try again.",show_alert=True)
            except: pass
    ...
    def handle_left_chat_member(message):
        try:
            if message.left_chat_member.id==bot_obj.get_me().id:
                pass # Group data deletion removed
