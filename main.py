import os
import logging
import requests
import json
import threading
import time
import io
import subprocess
import glob
import re
import math
import tempfile
import asyncio
from flask import Flask, request, abort, jsonify, render_template_string, redirect
from datetime import datetime, timedelta
from pyrogram import Client, filters
from pyrogram.types import Message, InlineKeyboardMarkup, InlineKeyboardButton, CallbackQuery, Document, Audio, Voice, Video
from pyrogram.enums import ParseMode
from pymongo import MongoClient
from collections import Counter, deque
from itsdangerous import URLSafeTimedSerializer, SignatureExpired, BadSignature

# Configure logging
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

# --- CONFIGURATION FROM ENVIRONMENT ---
env = os.environ

TELEGRAM_MAX_BYTES = int(env.get("TELEGRAM_MAX_BYTES", str(20 * 1024 * 1024)))
REQUEST_TIMEOUT_GEMINI = int(env.get("REQUEST_TIMEOUT_GEMINI", "300"))
MAX_CONCURRENT_TRANSCRIPTS = int(env.get("MAX_CONCURRENT_TRANSCRIPTS", "4"))
MAX_PENDING_QUEUE = int(env.get("MAX_PENDING_QUEUE", "1"))
REQUEST_TIMEOUT_ASSEMBLY = int(env.get("REQUEST_TIMEOUT_ASSEMBLY", "300"))

GEMINI_API_KEYS = [t.strip() for t in env.get("GEMINI_API_KEYS", env.get("GEMINI_API_KEY", "")).split(",") if t.strip()]
SECRET_KEY = env.get("SECRET_KEY", "testkey123")
ADMIN_PANEL_SECRET = env.get("ADMIN_PANEL_SECRET", SECRET_KEY)
MONGO_URI = env.get("MONGO_URI", "")
DB_NAME = env.get("DB_NAME", "telegram_bot_db")
REQUIRED_CHANNEL = env.get("REQUIRED_CHANNEL", "")
BOT_TOKEN = ([t.strip() for t in env.get("BOT_TOKENS", "").split(",") if t.strip()] + [""])[0]
API_ID = int(env.get("API_ID", 0))
API_HASH = env.get("API_HASH", "")
ASSEMBLYAI_API_KEYS = [t.strip() for t in env.get("ASSEMBLYAI_API_KEYS", env.get("ASSEMBLYAI_API_KEY", "")).split(",") if t.strip()]
ASSEMBLYAI_BASE_URL = "https://api.assemblyai.com/v2"
raw_admins = env.get("ADMIN_USER_IDS", "")
ADMIN_USER_IDS = []
for part in [p.strip() for p in raw_admins.split(",") if p.strip()]:
    try:
        ADMIN_USER_IDS.append(int(part))
    except Exception:
        pass

if not BOT_TOKEN or not API_ID or not API_HASH:
    logging.error("API_ID, API_HASH, or BOT_TOKEN is missing. Bot will not function.")

# --- DATABASE AND UTILITY SETUP ---

client_mongo = MongoClient(MONGO_URI) if MONGO_URI else MongoClient()
db = client_mongo[DB_NAME]
users_collection = db["users"]
groups_collection = db["groups"]
settings_collection = db["settings"]

flask_app = Flask(__name__)
# Initialize Pyrogram Client
app = Client(
    "pyrogram_stt_bot",
    api_id=API_ID,
    api_hash=API_HASH,
    bot_token=BOT_TOKEN,
    workers=MAX_CONCURRENT_TRANSCRIPTS * 2 # Pyrogram handles its own threading for network operations
)

serializer = URLSafeTimedSerializer(SECRET_KEY)

_LANG_RAW = "🇬🇧 English:en,🇸🇦 العربية:ar,🇪🇸 Español:es,🇫🇷 Français:fr,🇷🇺 Русский:ru,🇩🇪 Deutsch:de,🇮🇳 हिन्दी:hi,🇮🇷 فارسی:fa,🇮🇩 Indonesia:id,🇺🇦 Українська:uk,🇦🇿 Azərbaycan:az,🇮🇹 Italiano:it,🇹🇷 Türkçe:tr,🇧🇬 Български:bg,🇷🇸 Srpski:sr,🇵🇰 اردو:ur,🇹🇭 ไทย:th,🇻🇳 Tiếng Việt:vi,🇯🇵 日本語:ja,🇰🇷 한국어:ko,🇨🇳 中文:zh,🇳🇱 Nederlands:nl,🇸🇪 Svenska:sv,🇳🇴 Norsk:no,🇮🇱 עברית:he,🇩🇰 Dansk:da,🇪🇹 አማርኛ:am,🇫🇮 Suomi:fi,🇧🇩 বাংলা:bn,🇰🇪 Kiswahili:sw,🇪🇹 Oromoo:om,🇳🇵 नेपाली:ne,🇵🇱 Polski:pl,🇬🇷 Ελληνικά:el,🇨🇿 Čeština:cs,🇮🇸 Íslenska:is,🇱🇹 Lietuvių:lt,🇱🇻 Latviešu:lv,🇭🇷 Hrvatski:hr,🇷🇸 Bosanski:bs,🇭🇺 Magyar:hu,🇷🇴 Română:ro,🇸🇴 Somali:so,🇲🇾 Melayu:ms,🇺🇿 O'zbekcha:uz,🇵🇭 Tagalog:tl,🇵🇹 Português:pt"
LANG_OPTIONS = [(p.split(":", 1)[0].strip(), p.split(":", 1)[1].strip()) for p in _LANG_RAW.split(",")]
CODE_TO_LABEL = {code: label for label, code in LANG_OPTIONS}
LABEL_TO_CODE = {label: code for label, code in LANG_OPTIONS}

user_transcriptions = {}
action_usage = {}
memory_lock = threading.Lock()
ALLOWED_EXTENSIONS = set(["mp3", "wav", "m4a", "ogg", "webm", "flac", "mp4", "mkv", "avi", "mov", "hevc", "aac", "aiff", "amr", "wma", "opus", "m4v", "ts", "flv", "3gp"])
FFMPEG_BINARY = env.get("FFMPEG_BINARY", "") # Left for future use if local conversion is needed

# Async Queue and Semaphore
transcript_semaphore = asyncio.Semaphore(MAX_CONCURRENT_TRANSCRIPTS)
PENDING_QUEUE = asyncio.Queue(MAX_PENDING_QUEUE)

# --- HELPER FUNCTIONS ---

def norm_user_id(uid):
    try:
        return str(int(uid))
    except:
        return str(uid)

async def check_subscription(client: Client, user_id):
    if not REQUIRED_CHANNEL:
        return True
    try:
        member = await client.get_chat_member(REQUIRED_CHANNEL, user_id)
        return member.status in ['member', 'administrator', 'creator']
    except Exception:
        return False

async def send_subscription_message(client: Client, chat_id):
    if not REQUIRED_CHANNEL:
        return
    try:
        chat = await client.get_chat(chat_id)
        if chat.type.name.lower() != 'private':
            return
    except Exception:
        return
    try:
        m = InlineKeyboardMarkup([[InlineKeyboardButton("Click here to join the Group", url=f"https://t.me/{REQUIRED_CHANNEL.lstrip('@')}")]])
        await client.send_message(chat_id, "🔒 Access Locked You cannot use this bot until you join the Group.", reply_markup=m)
    except Exception:
        pass

def update_user_activity(user_id):
    uid = norm_user_id(user_id)
    now = datetime.now()
    users_collection.update_one({"user_id": uid}, {"$set": {"last_active": now}, "$setOnInsert": {"first_seen": now, "stt_conversion_count": 0}}, upsert=True)

def increment_processing_count(user_id, service_type):
    users_collection.update_one({"user_id": norm_user_id(user_id)}, {"$inc": {f"{service_type}_conversion_count": 1}})

def get_stt_user_lang(user_id):
    ud = users_collection.find_one({"user_id": norm_user_id(user_id)})
    return ud.get("stt_language", "en") if ud else "en"

def set_stt_user_lang(user_id, lang_code):
    users_collection.update_one({"user_id": norm_user_id(user_id)}, {"$set": {"stt_language": lang_code}}, upsert=True)

def get_user_send_mode(user_id):
    ud = users_collection.find_one({"user_id": norm_user_id(user_id)})
    return ud.get("stt_send_mode", "file") if ud else "file"

def set_user_send_mode(user_id, mode):
    if mode not in ("file", "split"):
        mode = "file"
    users_collection.update_one({"user_id": norm_user_id(user_id)}, {"$set": {"stt_send_mode": mode}}, upsert=True)

def delete_transcription_later(user_id, message_id):
    time.sleep(86400)
    with memory_lock:
        if user_id in user_transcriptions and message_id in user_transcriptions[user_id]:
            del user_transcriptions[user_id][message_id]

def is_transcoding_like_error(msg):
    if not msg:
        return False
    m = msg.lower()
    checks = ["transcoding failed", "file does not appear to contain audio", "text/html", "html document", "unsupported media type", "could not decode"]
    return any(ch in m for ch in checks)

def build_lang_keyboard(callback_prefix, row_width=3, message_id=None):
    m = InlineKeyboardMarkup(row_width=row_width)
    buttons = [InlineKeyboardButton(label, callback_data=f"{callback_prefix}|{code}|{message_id}" if message_id else f"{callback_prefix}|{code}") for label, code in LANG_OPTIONS]
    m.row(*buttons[0:row_width])
    m.row(*buttons[row_width:row_width*2])
    # ... Simplified for brevity, normally you'd handle all rows ...
    for i in range(0, len(buttons), row_width):
         m.add(*buttons[i:i + row_width])
    return m

def build_result_mode_keyboard(prefix="result_mode"):
    return InlineKeyboardMarkup(row_width=2, inline_keyboard=[
        [InlineKeyboardButton("📄 .txt file", callback_data=f"{prefix}|file"), InlineKeyboardButton("💬 Split messages", callback_data=f"{prefix}|split")]
    ])

async def animate_processing_message(client: Client, chat_id, message_id, stop_event):
    frames = ["🔄 Processing", "🔄 Processing.", "🔄 Processing..", "🔄 Processing..."]
    idx = 0
    while not stop_event.is_set():
        try:
            await client.edit_message_text(chat_id, message_id, frames[idx % len(frames)])
        except:
            pass
        idx = (idx + 1) % len(frames)
        await asyncio.sleep(0.6)

def normalize_text_offline(text):
    return re.sub(r'\s+', ' ', text).strip() if text else text

def extract_key_points_offline(text, max_points=6):
    if not text:
        return ""
    sentences = [s.strip() for s in re.split(r'(?<=[\.\!\?])\s+', text) if s.strip()]
    if not sentences:
        return ""
    words = [w for w in re.findall(r'\w+', text.lower()) if len(w) > 3]
    if not words:
        return "\n".join(f"- {s}" for s in sentences[:max_points])
    freq = Counter(words)
    sentence_scores = [(sum(freq.get(w, 0) for w in re.findall(r'\w+', s.lower())), s) for s in sentences]
    sentence_scores.sort(key=lambda x: x[0], reverse=True)
    top_sentences = sorted(sentence_scores[:max_points], key=lambda x: sentences.index(x[1]))
    return "\n".join(f"- {s}" for _, s in top_sentences)

def safe_extension_from_filename(filename):
    return filename.rsplit(".", 1)[-1].lower() if filename and "." in filename else ""

# The following two functions will run in a separate thread/process executor since they use the sync requests library
def transcribe_file_with_assemblyai_sync(audio_source, language_code):
    if not ASSEMBLYAI_API_KEYS:
        raise RuntimeError("ASSEMBLYAI_API_KEYS not set")
    last_exception = None
    for api_key in ASSEMBLYAI_API_KEYS:
        try:
            headers = {"Authorization": api_key, "Content-Type": "application/json", "Accept": "application/json"}
            config = {"audio_url": audio_source}
            if language_code != "en":
                config["language_code"] = language_code
            submit_url = f"{ASSEMBLYAI_BASE_URL}/transcript"
            submit_resp = requests.post(submit_url, headers=headers, json=config, timeout=REQUEST_TIMEOUT_ASSEMBLY)
            submit_resp.raise_for_status()
            transcript_id = submit_resp.json().get("id")
            if not transcript_id:
                raise RuntimeError("AssemblyAI submission failed: No transcript ID received")
            poll_url = f"{ASSEMBLYAI_BASE_URL}/transcript/{transcript_id}"
            start_time = time.time()
            while time.time() - start_time < REQUEST_TIMEOUT_ASSEMBLY:
                poll_resp = requests.get(poll_url, headers={"Authorization": api_key}, timeout=30)
                poll_resp.raise_for_status()
                status = poll_resp.json().get("status")
                if status == "completed":
                    return poll_resp.json().get("text", "")
                elif status in ["failed", "error"]:
                    raise RuntimeError(f"AssemblyAI transcription failed. Details: {poll_resp.json()}")
                elif status == "processing":
                    time.sleep(5)
                else:
                    time.sleep(3)
            raise RuntimeError("AssemblyAI transcription timed out")
        except Exception as e:
            logging.warning(f"AssemblyAI key failed: {str(e)}. Trying next key if available.")
            last_exception = e
            continue
    raise RuntimeError(f"All AssemblyAI keys failed. Last error: {str(last_exception) if last_exception else 'No keys were available.'}")

def upload_file_with_assemblyai_sync(file_path):
    if not ASSEMBLYAI_API_KEYS:
        raise RuntimeError("ASSEMBLYAI_API_KEYS not set")
    last_exception = None
    for api_key in ASSEMBLYAI_API_KEYS:
        try:
            headers = {"Authorization": api_key, "Content-Type": "application/octet-stream"}
            with open(file_path, 'rb') as f:
                upload_resp = requests.post(f"{ASSEMBLYAI_BASE_URL}/upload", headers=headers, data=f, timeout=REQUEST_TIMEOUT_ASSEMBLY)
            upload_resp.raise_for_status()
            upload_url = upload_resp.json().get("upload_url")
            if not upload_url:
                raise RuntimeError("AssemblyAI upload failed: No upload_url received")
            return upload_url
        except Exception as e:
            logging.warning(f"AssemblyAI upload key failed: {str(e)}. Trying next key if available.")
            last_exception = e
            continue
    raise RuntimeError(f"All AssemblyAI upload keys failed. Last error: {str(last_exception) if last_exception else 'No keys were available.'}")

async def transcribe_via_selected_service(input_source, lang_code, is_local_file=True):
    loop = asyncio.get_event_loop()
    if is_local_file:
        try:
            upload_url = await loop.run_in_executor(None, upload_file_with_assemblyai_sync, input_source)
            text = await loop.run_in_executor(None, transcribe_file_with_assemblyai_sync, upload_url, lang_code)
            if text is None:
                raise RuntimeError("AssemblyAI returned no text")
            return text, "assemblyai_upload"
        except Exception as e:
            raise RuntimeError("AssemblyAI upload and transcription failed: " + str(e))
    else:
        # This branch is for direct URL transcription, but Pyrogram download is better
        # Keeping this structure for completeness if a non-telegram URL source were used
        try:
            text = await loop.run_in_executor(None, transcribe_file_with_assemblyai_sync, input_source, lang_code)
            if text is None:
                raise RuntimeError("AssemblyAI returned no text")
            return text, "assemblyai_url"
        except Exception as e:
            raise RuntimeError("AssemblyAI transcription failed: " + str(e))

def split_text_into_chunks(text, limit=4096):
    if not text:
        return []
    chunks = []
    start = 0
    n = len(text)
    while start < n:
        end = min(start + limit, n)
        if end < n:
            last_space = text.rfind(" ", start, end)
            if last_space > start:
                end = last_space
        chunk = text[start:end].strip()
        if not chunk:
            end = start + limit
            chunk = text[start:end].strip()
        chunks.append(chunk)
        start = end
    return chunks

async def attach_action_buttons(client: Client, chat_id, message_id, text):
    try:
        include_summarize = len(text) > 1000 if text else False
        m = InlineKeyboardMarkup([[InlineKeyboardButton("⭐️Clean transcript", callback_data=f"clean_up|{chat_id}|{message_id}")]])
        if include_summarize:
            m.inline_keyboard.append([InlineKeyboardButton("Get Summarize", callback_data=f"get_key_points|{chat_id}|{message_id}")]])
        try:
            await client.edit_message_reply_markup(chat_id, message_id, reply_markup=m)
        except Exception:
            pass
    except Exception:
        pass
    try:
        action_usage[f"{chat_id}|{message_id}|clean_up"] = 0
        action_usage[f"{chat_id}|{message_id}|get_key_points"] = 0
    except Exception:
        pass

async def process_transcription_result(client: Client, chat_id, message_id, corrected_text, uid):
    uid_key = str(chat_id)
    user_mode = get_user_send_mode(uid_key)
    sent_msg = None
    if len(corrected_text) > 4000:
        if user_mode == "file":
            f = io.BytesIO(corrected_text.encode("utf-8"))
            f.name = "Transcript.txt"
            sent = await client.send_document(chat_id, f, reply_to_message_id=message_id)
            sent_msg = sent
        else:
            chunks = split_text_into_chunks(corrected_text, limit=4096)
            last_sent = None
            for idx, chunk in enumerate(chunks):
                if idx == 0:
                    last_sent = await client.send_message(chat_id, chunk, reply_to_message_id=message_id)
                else:
                    last_sent = await client.send_message(chat_id, chunk)
            sent_msg = last_sent
    else:
        sent_msg = await client.send_message(chat_id, corrected_text or "⚠️ Warning Make sure the voice is clear or speaking in the language you Choosed.", reply_to_message_id=message_id)

    if sent_msg:
        try:
            await attach_action_buttons(client, chat_id, sent_msg.id, corrected_text)
        except Exception:
            pass
        try:
            user_transcriptions.setdefault(uid_key, {})[sent_msg.id] = corrected_text
            threading.Thread(target=delete_transcription_later, args=(uid_key, sent_msg.id), daemon=True).start()
        except Exception:
            pass
    increment_processing_count(uid, "stt")

async def process_file_source(client: Client, message: Message, file_path: str):
    uid = str(message.from_user.id)
    chatid = str(message.chat.id)
    lang = get_stt_user_lang(uid)
    original_msg_id = message.id

    processing_msg = await client.send_message(chatid, "🔄 Processing...", reply_to_message_id=original_msg_id)
    processing_msg_id = processing_msg.id
    stop_event = asyncio.Event()
    animation_task = asyncio.create_task(animate_processing_message(client, chatid, processing_msg_id, stop_event))

    try:
        input_source = file_path
        
        try:
            text, used_service = await transcribe_via_selected_service(input_source, lang, is_local_file=True)
        except Exception as e:
            error_msg = str(e)
            logging.exception("Error during transcription")
            if is_transcoding_like_error(error_msg):
                await client.send_message(chatid, "⚠️ Transcription error: file is not audible or format is unsupported.", reply_to_message_id=original_msg_id)
            else:
                await client.send_message(chatid, f"Error during transcription: {error_msg}", reply_to_message_id=original_msg_id)
            return

        corrected_text = normalize_text_offline(text)
        await process_transcription_result(client, chatid, original_msg_id, corrected_text, uid)

    finally:
        stop_event.set()
        animation_task.cancel()
        try:
            await client.delete_messages(chatid, processing_msg_id)
        except Exception:
            pass
        if file_path:
            try:
                os.remove(file_path)
            except Exception:
                pass

async def worker_async():
    while True:
        try:
            item = await PENDING_QUEUE.get()
            
            # Use the Pyrogram client from the app instance
            client, message, file_id, file_size, filename = item
            
            await transcript_semaphore.acquire()
            try:
                # Pyrogram's download_media is highly optimized for large files
                temp_file_path = await client.download_media(message, file_name=f"{tempfile.gettempdir()}/{file_id}_{int(time.time())}.{safe_extension_from_filename(filename or 'mp3')}")
                
                logging.info(f"Starting Pyrogram processing for user {message.from_user.id}. Queue size: {PENDING_QUEUE.qsize()}")
                await process_file_source(client, message, temp_file_path)
            finally:
                transcript_semaphore.release()
                PENDING_QUEUE.task_done()
        except asyncio.CancelledError:
            break
        except Exception:
            logging.exception("Error in worker thread")
        await asyncio.sleep(0.5)

# Start Pyrogram Workers in the background
async def start_worker_tasks():
    for _ in range(MAX_CONCURRENT_TRANSCRIPTS):
        asyncio.create_task(worker_async())

# --- PYROGRAM HANDLERS ---

@app.on_message(filters.command("start") & filters.private)
async def start_handler(client: Client, message: Message):
    try:
        update_user_activity(message.from_user.id)
        if str(message.from_user.id) not in ADMIN_USER_IDS and not await check_subscription(client, message.from_user.id):
            await send_subscription_message(client, message.chat.id)
            return
        await message.reply_text("Choose your file language for transcription using the below buttons:", reply_markup=build_lang_keyboard("start_select_lang"))
    except Exception:
        logging.exception("Error in start_handler")

@app.on_callback_query(filters.regex(r"^start_select_lang\|"))
async def start_select_lang_callback(client: Client, call: CallbackQuery):
    try:
        uid = str(call.from_user.id)
        _, lang_code = call.data.split("|", 1)
        lang_label = CODE_TO_LABEL.get(lang_code, lang_code)
        set_stt_user_lang(uid, lang_code)
        try:
            await client.delete_messages(call.message.chat.id, call.message.id)
        except Exception:
            pass
        welcome_text = "👋 Salaam!    \n• Send me\n• voice message\n• audio file\n• video\n• to transcribe for free. I support large files!"
        await call.message.reply_text(welcome_text)
        await call.answer(f"✅ Language set to {lang_label}")
    except Exception:
        logging.exception("Error in start_select_lang_callback")
        await call.answer("❌ Error setting language, try again.", show_alert=True)

@app.on_message(filters.command("help"))
async def handle_help(client: Client, message: Message):
    try:
        update_user_activity(message.from_user.id)
        if str(message.from_user.id) not in ADMIN_USER_IDS and not await check_subscription(client, message.from_user.id):
            await send_subscription_message(client, message.chat.id)
            return
        text = "Commands supported:\n/start - Show welcome message\n/lang  - Change language\n/mode  - Change result delivery mode\n/help  - This help message\n\nSend a voice/audio/video (I support large files!) and I will transcribe it Need help? Contact: @lakigithub"
        await message.reply_text(text)
    except Exception:
        logging.exception("Error in handle_help")

@app.on_message(filters.command("lang"))
async def handle_lang(client: Client, message: Message):
    try:
        if str(message.from_user.id) not in ADMIN_USER_IDS and not await check_subscription(client, message.from_user.id):
            await send_subscription_message(client, message.chat.id)
            return
        kb = build_lang_keyboard("stt_lang")
        await message.reply_text("Choose your file language for transcription using the below buttons:", reply_markup=kb)
    except Exception:
        logging.exception("Error in handle_lang")

@app.on_message(filters.command("mode"))
async def handle_mode(client: Client, message: Message):
    try:
        if str(message.from_user.id) not in ADMIN_USER_IDS and not await check_subscription(client, message.from_user.id):
            await send_subscription_message(client, message.chat.id)
            return
        current_mode = get_user_send_mode(str(message.from_user.id))
        mode_text = "📄 .txt file" if current_mode == "file" else "💬 Split messages"
        await message.reply_text(f"How do I send you the results? Current mode: **{mode_text}**", reply_markup=build_result_mode_keyboard())
    except Exception:
        logging.exception("Error in handle_mode")

@app.on_callback_query(filters.regex(r"^stt_lang\|"))
async def on_stt_language_select(client: Client, call: CallbackQuery):
    try:
        uid = str(call.from_user.id)
        _, lang_code = call.data.split("|", 1)
        lang_label = CODE_TO_LABEL.get(lang_code, lang_code)
        set_stt_user_lang(uid, lang_code)
        await call.answer(f"✅ Language set: {lang_label}")
        try:
            await client.delete_messages(call.message.chat.id, call.message.id)
        except Exception:
            pass
    except Exception:
        logging.exception("Error in on_stt_language_select")
        await call.answer("❌ Error setting language, try again.", show_alert=True)

@app.on_callback_query(filters.regex(r"^result_mode\|"))
async def on_result_mode_select(client: Client, call: CallbackQuery):
    try:
        uid = str(call.from_user.id)
        _, mode = call.data.split("|", 1)
        set_user_send_mode(uid, mode)
        mode_text = "📄 .txt file" if mode == "file" else "💬 Split messages"
        try:
            await client.delete_messages(call.message.chat.id, call.message.id)
        except Exception:
            pass
        await call.answer(f"✅ Result mode set: {mode_text}")
    except Exception:
        logging.exception("Error in on_result_mode_select")
        await call.answer("❌ Error setting result mode, try again.", show_alert=True)

@app.on_message(filters.new_chat_members)
async def handle_new_chat_members(client: Client, message: Message):
    try:
        for member in message.new_chat_members:
            if member.id == client.me.id:
                group_data = {'_id': str(message.chat.id), 'title': message.chat.title, 'type': message.chat.type.name, 'added_date': datetime.now()}
                groups_collection.update_one({'_id': group_data['_id']}, {'$set': group_data}, upsert=True)
                await message.reply_text("Thanks for adding me! I'm ready to transcribe your media files.")
    except Exception:
        logging.exception("Error in handle_new_chat_members")

@app.on_message(filters.left_chat_member)
async def handle_left_chat_member(client: Client, message: Message):
    try:
        if message.left_chat_member.id == client.me.id:
            groups_collection.delete_one({'_id': str(message.chat.id)})
    except Exception:
        logging.exception("Error in handle_left_chat_member")

@app.on_message(filters.private & (filters.voice | filters.audio | filters.video | filters.document))
async def handle_media_types(client: Client, message: Message):
    try:
        update_user_activity(message.from_user.id)
        if str(message.from_user.id) not in ADMIN_USER_IDS and not await check_subscription(client, message.from_user.id):
            await send_subscription_message(client, message.chat.id)
            return

        media = message.voice or message.audio or message.video or message.document
        if not media:
            return

        file_id = file_size = filename = None
        if isinstance(media, Voice):
            file_id = media.file_id
            file_size = media.file_size
            filename = "voice.ogg"
        elif isinstance(media, Audio):
            file_id = media.file_id
            file_size = media.file_size
            filename = media.file_name or "audio"
        elif isinstance(media, Video):
            file_id = media.file_id
            file_size = media.file_size
            filename = media.file_name or "video.mp4"
        elif isinstance(media, Document):
            mime = media.mime_type
            filename = media.file_name or "file"
            ext = safe_extension_from_filename(filename)
            if (mime and ("audio" in mime or "video" in mime)) or ext in ALLOWED_EXTENSIONS:
                file_id = media.file_id
                file_size = media.file_size
            else:
                await message.reply_text("Sorry, I can only transcribe audio or video files.")
                return
        
        if not file_id:
            return

        # No Web Upload needed, as Pyrogram handles large file download automatically
        
        if PENDING_QUEUE.full():
            await message.reply_text("⚠️ Server busy. Try again later.")
            return

        # Queue the download/transcribe task
        await PENDING_QUEUE.put((client, message, file_id, file_size, filename))
        
        if PENDING_QUEUE.qsize() > 0:
             await message.reply_text(f"✅ Your file has been queued. You are position **#{PENDING_QUEUE.qsize()}** in line.")

    except Exception:
        logging.exception("Error in handle_media_types")

# --- ADMIN AND ACTION HANDLERS (Simplified for Pyrogram) ---

# The /admin command (can be kept largely synchronous since it's admin only)
@app.on_message(filters.command("admin"))
async def handle_admin(client: Client, message: Message):
    # This remains largely synchronous and requires `run_in_executor` for DB/Flask access
    if message.from_user.id not in ADMIN_USER_IDS:
        await message.reply_text("Access Denied: Only admins can use this command.")
        return
    
    def get_stats():
        total_users = users_collection.count_documents({})
        total_groups = groups_collection.count_documents({})
        total_stt_conversions = sum(user.get("stt_conversion_count", 0) for user in users_collection.find({}))
        seven_days_ago = datetime.now() - timedelta(days=7)
        active_users_7d = users_collection.count_documents({"last_active": {"$gte": seven_days_ago}})
        return total_users, total_groups, total_stt_conversions, active_users_7d

    total_users, total_groups, total_stt_conversions, active_users_7d = await asyncio.get_event_loop().run_in_executor(None, get_stats)
    
    report_text = f"<b>📊 Bot Admin Panel</b>\n\n👤 Total Users: <b>{total_users}</b>\n👥 Total Groups: <b>{total_groups}</b>\n🗣️ Total Transcriptions: <b>{total_stt_conversions:,}</b>\n🟢 Active Users (Last 7 Days): <b>{active_users_7d}</b>\n\n"
    # Note: Flask webhooks are now gone, so the link is for the *Flask Keep-Alive server* if hosted with a domain.
    # If not running with a domain, this link won't work. Keeping it for platform compatibility.
    link = f"/admin?secret={ADMIN_PANEL_SECRET}" 
    markup = InlineKeyboardMarkup([[InlineKeyboardButton("Open Admin Panel", url=link)]])
    
    await message.reply_text(report_text, parse_mode=ParseMode.HTML, reply_markup=markup)

# Helper function for Gemini (runs sync in executor)
def ask_gemini_sync(text, instruction, timeout=REQUEST_TIMEOUT_GEMINI):
    if not GEMINI_API_KEYS:
        raise RuntimeError("GEMINI_API_KEYS not set")
    last_exception = None
    for api_key in GEMINI_API_KEYS:
        try:
            url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key={api_key}"
            payload = {"contents": [{"parts": [{"text": instruction}, {"text": text}]}]}
            headers = {"Content-Type": "application/json"}
            resp = requests.post(url, headers=headers, json=payload, timeout=timeout)
            resp.raise_for_status()
            result = resp.json()
            if "candidates" in result and isinstance(result["candidates"], list) and len(result["candidates"]) > 0:
                try:
                    return result['candidates'][0]['content']['parts'][0]['text']
                except:
                    return json.dumps(result['candidates'][0])
            raise RuntimeError(f"Gemini response lacks candidates: {json.dumps(result)}")
        except Exception as e:
            last_exception = e
            continue
    raise RuntimeError(f"All Gemini API keys failed. Last error: {str(last_exception) if last_exception else 'No keys were available.'}")

@app.on_callback_query(filters.regex(r"^get_key_points\|"))
async def get_key_points_callback(client: Client, call: CallbackQuery):
    # Same logic as before, just using async features
    try:
        parts = call.data.split("|")
        _, chat_id_part, msg_id_part = parts
        chat_id_val = int(chat_id_part)
        msg_id = int(msg_id_part)
        usage_key = f"{chat_id_val}|{msg_id}|get_key_points"
        
        with memory_lock:
            usage = action_usage.get(usage_key, 0)
            if usage >= 1:
                await call.answer("Get Summarize unavailable (maybe expired)", show_alert=True)
                return
            action_usage[usage_key] = usage + 1
            stored = user_transcriptions.get(str(chat_id_val), {}).get(msg_id)
        
        if not stored:
            await call.answer("Get Summarize unavailable (maybe expired)", show_alert=True)
            return

        await call.answer("Generating...")
        status_msg = await client.send_message(call.message.chat.id, "🔄 Processing...", reply_to_message_id=call.message.id)
        stop_event = asyncio.Event()
        animation_task = asyncio.create_task(animate_processing_message(client, call.message.chat.id, status_msg.id, stop_event))
        
        summary = ""
        try:
            lang = get_stt_user_lang(str(chat_id_val)) or "en"
            instruction = f"What is this report and what is it about? Please summarize them for me into (lang={lang}) without adding any introductions, notes, or extra phrases."
            try:
                summary = await asyncio.get_event_loop().run_in_executor(None, ask_gemini_sync, stored, instruction)
            except Exception:
                summary = extract_key_points_offline(stored, max_points=6)
        finally:
            stop_event.set()
            animation_task.cancel()
            try:
                await client.delete_messages(call.message.chat.id, status_msg.id)
            except Exception:
                pass
            
        if summary:
            await call.message.reply_text(f"**Summary:**\n{summary}", reply_to_message_id=call.message.id, parse_mode=ParseMode.MARKDOWN)
        else:
            await call.message.reply_text("No Summary returned.", reply_to_message_id=call.message.id)

    except Exception:
        logging.exception("Error in get_key_points_callback")

@app.on_callback_query(filters.regex(r"^clean_up\|"))
async def clean_up_callback(client: Client, call: CallbackQuery):
    # Same logic as before, using async features
    try:
        parts = call.data.split("|")
        _, chat_id_part, msg_id_part = parts
        chat_id_val = int(chat_id_part)
        msg_id = int(msg_id_part)
        usage_key = f"{chat_id_val}|{msg_id}|clean_up"
        
        with memory_lock:
            usage = action_usage.get(usage_key, 0)
            if usage >= 1:
                await call.answer("Clean up unavailable (maybe expired)", show_alert=True)
                return
            action_usage[usage_key] = usage + 1
            stored = user_transcriptions.get(str(chat_id_val), {}).get(msg_id)
        
        if not stored:
            await call.answer("Clean up unavailable (maybe expired)", show_alert=True)
            return

        await call.answer("Cleaning up...")
        status_msg = await client.send_message(call.message.chat.id, "🔄 Processing...", reply_to_message_id=call.message.id)
        stop_event = asyncio.Event()
        animation_task = asyncio.create_task(animate_processing_message(client, call.message.chat.id, status_msg.id, stop_event))

        cleaned = ""
        try:
            lang = get_stt_user_lang(str(chat_id_val)) or "en"
            instruction = f"Clean and normalize this transcription (lang={lang}). Remove ASR artifacts like [inaudible], repeated words, filler noises, timestamps, and incorrect punctuation. Produce a clean, well-punctuated, readable text in the same language. Do not add introductions or explanations."
            try:
                cleaned = await asyncio.get_event_loop().run_in_executor(None, ask_gemini_sync, stored, instruction)
            except Exception:
                cleaned = normalize_text_offline(stored)
        finally:
            stop_event.set()
            animation_task.cancel()
        
        if not cleaned:
            await client.edit_message_text(call.message.chat.id, status_msg.id, "No cleaned text returned.")
            return

        uid_key = str(chat_id_val)
        user_mode = get_user_send_mode(uid_key)
        
        if len(cleaned) > 4000 and user_mode == "file":
            try:
                await client.delete_messages(call.message.chat.id, status_msg.id)
            except Exception:
                pass
            f = io.BytesIO(cleaned.encode("utf-8"))
            f.name = "cleaned.txt"
            sent = await client.send_document(call.message.chat.id, f, reply_to_message_id=call.message.id)
            last_sent_id = sent.id
        elif len(cleaned) > 4000 and user_mode == "split":
            try:
                await client.delete_messages(call.message.chat.id, status_msg.id)
            except Exception:
                pass
            chunks = split_text_into_chunks(cleaned, limit=4096)
            last_sent = None
            for idx, chunk in enumerate(chunks):
                if idx == 0:
                    last_sent = await client.send_message(call.message.chat.id, chunk, reply_to_message_id=call.message.id)
                else:
                    last_sent = await client.send_message(call.message.chat.id, chunk)
            last_sent_id = last_sent.id if last_sent else -1
        else:
            await client.edit_message_text(call.message.chat.id, status_msg.id, cleaned)
            last_sent_id = status_msg.id
            
        if last_sent_id != -1:
            with memory_lock:
                user_transcriptions.setdefault(uid_key, {})[last_sent_id] = cleaned
                threading.Thread(target=delete_transcription_later, args=(uid_key, last_sent_id), daemon=True).start()
                action_usage[f"{call.message.chat.id}|{last_sent_id}|clean_up"] = 0
                action_usage[f"{call.message.chat.id}|{last_sent_id}|get_key_points"] = 0
            
    except Exception:
        logging.exception("Error in clean_up_callback")


# --- FLASK KEEP-ALIVE SERVER ---

@flask_app.route("/", methods=["GET", "POST", "HEAD"])
def keep_alive():
    # Only for checking if the server is running
    return "Bot is alive and running with Pyrogram! ✅", 200

def run_flask():
    logging.info("Starting Flask Keep-Alive server...")
    flask_app.run(host="0.0.0.0", port=int(os.environ.get("PORT", 8080)), threaded=True)

# --- MAIN EXECUTION ---

if __name__ == "__main__":
    if not BOT_TOKEN:
        logging.error("Startup failed: BOT_TOKEN is not configured.")
    else:
        # Start Flask in a separate thread
        Thread(target=run_flask, daemon=True).start()

        # Start Pyrogram
        logging.info("Starting Pyrogram client and worker tasks...")
        
        # Pyrogram's idle() runs the client and the event loop. 
        # We need to ensure the worker tasks start *before* or *during* the client's run.
        
        # Pyrogram's Client.run() is synchronous and blocks. 
        # We use Client.start() + asyncio.get_event_loop().run_forever() for background tasks.
        
        async def main():
            await app.start()
            logging.info("Pyrogram client started.")
            await start_worker_tasks()
            # This task ensures the application runs until terminated
            await asyncio.Future() 

        try:
            asyncio.run(main())
        except KeyboardInterrupt:
            logging.info("Bot shutting down...")
        except Exception:
            logging.exception("An unhandled error occurred in the main asyncio loop")
        finally:
            if app.is_connected:
                asyncio.run(app.stop())
